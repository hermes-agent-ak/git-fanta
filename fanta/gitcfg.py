...
# alongside git-cola, so honouring that project's "cola." keys would let one
# ~/.gitconfig configure both applications at once.
CONFIG_PREFIX = 'fanta.'
...
class GitConfig(QtCore.QObject):
    ...
    def _key_candidates(self, key):
        """Yield the config keys to probe for `key`

        The key is tried as-is, then via the case-preserved name recorded while
        parsing (x.kittycat -> x.kittyCat), then lowercased.
        """
        yield key
